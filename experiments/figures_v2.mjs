...
export function prismaSvg(p) {
  const fmt = (x) =>
    x === null || x === undefined ? "NA (kayıt tutulmadı)" : `n = ${x}`;
  const boxes = [
    {
      x: 30,
      y: 50,
      ...
      label: [
        "IDENTIFICATION",
        `Açık web (Scholar/arXiv/DOI): ${fmt(p.identified_other)}`,
        `Veritabanı (yapılamadı): n = ${p.identified_db}`,
      ],
      fill: "#dde7f4",
    },
    {
      ...
      w: 280,
      h: 50,
      label: ["SCREENING — Duplikatlar çıkarıldı", fmt(p.duplicates_removed)],
      fill: "#dde7f4",
    },
    {
      x: 30,
      ...
      h: 60,
      label: [
        "SCREENING — Başlık/Özet incelemesi",
        fmt(p.screened),
        `Hariç: ${p.excluded_title_abstract === null ? "NA" : p.excluded_title_abstract}`,
      ],
      fill: "#dde7f4",
    },
    {
      ...
      label: [
        ...
        fmt(p.full_text_assessed),
        `Hariç: ${p.excluded_full_text === null ? "NA" : p.excluded_full_text}`,
      ],
      fill: "#dde7f4",
    },
    {
      ...
      h: 60,
      label: [
        "INCLUDED — Sentezde kullanılan",
        `n = ${p.included}`,
        "(Literatur_Notlari.pdf ile doğrulanır)",
      ],
      fill: "#bce5c8",
    },
  ];
  ...
  (p.exclusion_reasons_qualitative || p.exclusion_reasons || []).forEach(
    (r, i) => {
...
export function budgetCurveSvg(curve, budgets, metric = "tc") {
  ...
  const xMin = Math.log10(budgets[0]),
    xMax = Math.log10(budgets[budgets.length - 1]);
  ...
  for (const algo of Object.keys(curve)) {
    const pts = curve[algo].map((p) => ({
      x: xScale(p.budget),
      y: yScale(p[`${metric}_mean`]),
      yLo: yScale(Math.max(0, p[`${metric}_mean`] - p[`${metric}_sd`])),
      yHi: yScale(Math.min(1, p[`${metric}_mean`] + p[`${metric}_sd`])),
    }));
    // band
    const bandPath =
      "M " +
      pts.map((p) => `${p.x} ${p.yHi}`).join(" L ") +
      " L " +
      pts
        .slice()
        .reverse()
    ...
    // points
    for (const p of pts)
      svg += `<circle cx="${p.x}" cy="${p.y}" r="3" fill="${colors[algo]}"/>`;
  }
  // legend
  let lx = left;
  for (const algo of Object.keys(curve)) {
...
export function ruleCompletenessSvg(rb) {
  ...
  const bars = [
    {
      label: "B0 — Rule-based (7 imza)",
      val: m,
      color: "#c97a3d",
      text: `${rb.detectedByRules}/${rb.totalInvalid} = ${(m * 100).toFixed(1)}%`,
    },
    {
      label: "MDT — Spec oracle (δ)",
      val: 1.0,
      color: "#1d8b5d",
      text: `${rb.totalInvalid}/${rb.totalInvalid} = 100.0%`,
    },
  ];
  bars.forEach((b, i) => {
    const x = left + i * 320,
