...
const RULES = [
  ...
  {
    name: "R2_REPLAY",
    cond: (s, e) => e === "SEND_RELAY_DATA" && s === "CLOSED",
  },
  { name: "R3_GHOST", cond: (s, e) => e === "RECV_CREATED" && s === "IDLE" },
  {
    name: "R4_HSKIP",
    cond: (s, e) => e === "SEND_CREATE" && (s === "IDLE" || s === "CONNECTING"),
  },
  {
    name: "R5_PDATA",
    cond: (s, e) =>
      (e === "SEND_RELAY_DATA" || e === "RECV_RELAY_DATA") &&
      (s === "CREATE_SENT" || s === "CIRCUIT_BUILDING"),
  },
  {
    name: "R6_HIJACK",
    cond: (s, e) =>
      (e === "SEND_CREATE" ||
        ...
        e === "SEND_EXTEND" ||
        e === "RECV_EXTENDED") &&
      (s === "CIRCUIT_READY" || s === "TRANSMITTING"),
  },
  {
    name: "R7_CFLOOD",
    cond: (s, e) =>
      (e === "SEND_CREATE" || e === "RECV_CREATED") && s === "CREATE_SENT",
  },
];
...
function ruleBasedAudit() {
  ...
  for (const s of STATES)
    for (const e of EVENTS) {
      if (VALID[k(s, e)] === undefined)
        allInvalid.push({
          s,
          e,
          sev: classifyInvalid(s, e).severity,
          type: classifyInvalid(s, e).type,
        });
    }
  let detected = 0;
  ...
  for (const p of allInvalid) {
    ...
    if (hit) detected++;
      ...
      missedByType[p.type] = (missedByType[p.type] || 0) + 1;
      missedBySeverity[p.sev]++;
    }
  }
  return {
...
console.log(
  "[1/4] Latency ölçümü (overhead kalibrasyonu + Q×Σ batch benchmark)...",
);
...
function wilcoxonSignedRank(xs, ys) {
  if (xs.length !== ys.length)
    throw new Error("paired arrays must be equal length");
...
const prisma = {
  ...
  notes:
    "Ara sayılar (NA) raporlanmaz çünkü açık-web sürecinde audit-edilebilir log tutulmadı. Bu durum tezin sınırlılıklar bölümünde açıkça belirtilir.",
};
...
console.log("\nÖZET:");
console.log(
  `  Rule-based completeness: ${(ruleBased.completeness * 100).toFixed(1)}% (${ruleBased.detectedByRules}/${ruleBased.totalInvalid})`,
);
console.log(`  Missed by severity:`, ruleBased.missedBySeverity);
for (const pr of probes) {
  const d = latencyData[pr.label];
  console.log(
    `  Latency [${pr.label}]: ${d.mean_ns.toFixed(1)} ns/call (SD ${d.sd_ns.toFixed(1)}, p95 ${d.p95_ns.toFixed(1)})`,
  );
}
console.log(
  `  Wilcoxon B3 vs B1 [TC]: W=${wilcoxon.find((w) => w.a === "B3_MDT" && w.b === "B1_Random" && w.metric === "tc").W.toFixed(1)}, p=${wilcoxon.find((w) => w.a === "B3_MDT" && w.b === "B1_Random" && w.metric === "tc").p.toExponential(2)}`,
);
