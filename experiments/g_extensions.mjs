...
function rowKey(s, table) {
  return table.E.map((e) => (MQ(concat(s, e)) ? "1" : "0")).join("");
}
function fillTable(table) {
  const all = new Set([
    ...table.S,
    ...table.S.flatMap((s) => SIGMA.map((a) => s + a)),
  ]);
...
function isConsistent(table) {
  ...
  for (let i = 0; i < sList.length; i++)
    for (let j = i + 1; j < sList.length; j++) {
      ...
      for (const a of SIGMA) {
        const ri = table.T.get(sList[i] + a),
          rj = table.T.get(sList[j] + a);
        if (ri !== rj) {
          for (let p = 0; p < ri.length; p++)
            if (ri[p] !== rj[p]) return a + table.E[p];
        }
      }
    }
  return null;
...
function buildHypothesis(table) {
  ...
  for (const s of table.S)
    if (!stateMap.has(table.T.get(s))) stateMap.set(table.T.get(s), s);
  ...
  for (const s of states) {
    ...
    for (const a of SIGMA) {
      const sa = s + a;
      const rep =
        stateMap.get(table.T.get(sa)) ?? stateMap.get(rowKey(sa, table));
      delta[s][a] = rep;
    }
  }
...
function lStar() {
  ...
  for (let round = 0; round < 30; round++) {
    ...
    trace.push({
      round,
      S_size: table.S.length,
      E_size: table.E.length,
      hypStates: H.states.length,
      ce,
    });
    if (!ce) break;
...
const out = {
  config: {
    alphabet: SIGMA,
    eqMaxLen: 6,
    sutStates: Object.keys(SUT_DELTA).length,
    sutAccepting: [...ACCEPTING],
  },
  ...
  trace,
  counters: {
    membership_queries: mqCount,
    equivalence_queries: eqCount,
    runtime_ms: dtMs,
  },
  conclusion: [
...
await fs.writeFile(
  path.join(__dirname, "g_extensions.json"),
  JSON.stringify(out, null, 2),
);
console.log(
  `L* converged: ${trace.length} round(s), MQ=${mqCount}, EQ=${eqCount}, ${dtMs.toFixed(1)}ms`,
);
console.log(
  `Learned DFA states: ${H.states.length} (SUT: ${Object.keys(SUT_DELTA).length})`,
);
